						...
						<div className='flex flex-col gap-8 overflow-auto'>
							{DocsSections.map((section) => (
								<div key={section.title} className='flex flex-col gap-4'>
									<a
										className={`font-semibold text-lg hover:cursor-pointer ${isPathSelected(section.path) ? 'text-brand-gradient' : ''}`}
										onClick={() => {
											navigate('/docs' + section.path)
											setIsOpen(false)
										}}
									>
										{section.title}
