...
import { ApplicationName, NavMenuOptions } from '@/pages/landingPage/Content'
						...
						<div className='flex items-center justify-between gap-1 w-full'>
							<div className='flex items-center gap-4 w-full justify-between'>
								<a href='/'>
									<div className='flex items-center gap-2 text-2xl font-semibold'>
										...
										<span className={`${isDocsLayout ? 'hidden' : ''} md:flex`}>{ApplicationName}</span>
									</div>
								</a>
								<Button
									variant='outline'
							...
							<div className='hidden md:flex gap-1 md:items-center'>
								{NavMenuOptions.map((option) => (
									<Button variant='ghost' key={option.name} onClick={() => navigate(option.path)}>
										{option.name}
									</Button>
								))}
